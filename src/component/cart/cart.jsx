      ...
      if (response.success) {
        ...
        if (!isProduct) {
          setCartItems((prevItems) => ({
            ...prevItems,
            subcategory: prevItems.subcategory.filter(
              (item) => item.subcategoryId._id !== itemId
            ),
          }));
        }
      } else {
  ...
  if (CartData.length === 0) {
    return (
      <div className="d-flex flex-column align-items-center justify-content-center ">
        <h1 className="text-center textH">Shopping Cart</h1>
        <p className="text-center textP">Your cart is empty.</p>
        <hr className="horizantel" />
        <div className="textD">Back To Products and Continue Shopping</div>
      </div>
    );
  }
